...
export const getQuests = (data) => {
    return serviceAxios({
        url: `/v1/quests${data.id ? '/'+data.id : ''}${data.pageSize ? '?pageSize='+data.pageSize : ''}${data.page ? '&page='+data.page : ''}${data.type ? '?type='+data.type : ''}`,
        method: "get",
        data
    })
}
...
// 获取微信分享二维码
export const wechatShare = ({data, version}) => {
    return serviceAxios({
        url: `/v${version}/badge/submitClaimShare`,
        method: "post",
        data
    })
}
